...
export function CaptionStyler({
  ...
  return (
    <div className="flex flex-col gap-3 p-4">
      ...
      {/* Language */}
      <div className="flex flex-col gap-2">
        <span className="text-xs font-semibold uppercase tracking-widest" style={{ color: 'rgba(255,255,255,0.3)' }}>Language</span>
        <div className="flex gap-2">
          <div className="relative flex-1">
            <select
              ...
              onChange={e => {
                ...
                onChange({ language: val === 'original' ? null : val })
              }}
            ...
            <svg className="pointer-events-none absolute right-3" style={{ top: '50%', transform: 'translateY(-50%)' }} width="12" height="12" viewBox="0 0 12 12" fill="none">
          ...
          {currentLang !== 'original' && (
            <button
              onClick={() => onRetranscribe(currentLang)}
              disabled={retranscribing}
              className="px-3 py-2 rounded-lg text-xs font-semibold text-white disabled:opacity-50 shrink-0"
              style={{ background: retranscribing ? 'rgba(0,180,216,0.3)' : '#00b4d8', minWidth: 60 }}
            >
              {retranscribing ? (
        ...
        {currentLang !== 'original' && !retranscribing && (
          <p className="text-xs" style={{ color: 'rgba(255,255,255,0.3)' }}>
            Press Apply to re-transcribe in this language
          </p>
        )}
        {currentLang === 'original' && !retranscribing && (
          <button
            onClick={() => onRetranscribe('unknown')}
            className="text-xs px-2.5 py-1.5 rounded-lg font-semibold text-white w-full mt-1"
            style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.1)' }}
          >
            ↺ Re-transcribe for word-level sync
          </button>
        )}
        {retranscribing && (
          <p className="text-xs" style={{ color: '#00b4d8' }}>
            Re-transcribing… {retranscribeElapsed > 0 ? `${retranscribeElapsed}s` : 'starting'}
          </p>
        )}
